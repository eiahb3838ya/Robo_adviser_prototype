...
get_MVaR_CVaR_CVaR <- function(Ret_Data){
  ...
  if (is.data.frame(Ret_Data)) {
    ...
    if (is.xts(Ret_xts)) {
      ...
      ## Weight
      Wt_CVaR <- Fun_CVaR(Ret_xts[,-1], lmd = 0.5, alpha = 0.95)
      Wt_CVaR_DT <- cbind(round(Wt_CVaR, digits = 4), colnames(Ret_xts)[-1])
      ...
      RC_CVaR_DT <- as.data.table(cbind(RC_CVaR, colnames(Ret_xts)[-1]))
      ...
      MVaR_CVaR_DT <- cbind(MVaR_CVaR_DT, colnames(Ret_xts)[-1])
      colnames(MVaR_CVaR_DT) <- c("MVaR", "Strategy")
      
      CVaR_CVaR_DT <- as.data.table(CVaR_CVaR)
      CVaR_CVaR_DT <- cbind(CVaR_CVaR_DT, colnames(Ret_xts)[-1])
