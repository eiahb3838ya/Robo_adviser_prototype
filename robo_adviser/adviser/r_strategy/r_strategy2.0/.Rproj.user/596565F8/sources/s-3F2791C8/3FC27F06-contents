...
get_portfolio_result<-function(Equity_All){
  ...
  RL_CVaR <- get_RL_CVaR(Equity_All, Startn = RL_Period$Startn, Endn = RL_Period$Endn, Next_n = RL_Period$Next_n)
  RL_CVaR_Corr <- get_RL_Corr(RL_CVaR, col_name = colnames(Equity_All)[-1])
  RL_CVaR_Wt <- get_RL_CVaR_Wt(RL_CVaR, col_name = base::setdiff(colnames(Equity_All), "Ret")[-1])
  RL_CVaR_Ret <- get_RL_CVaR_Ret(RL_CVaR)
  # print("CVaR_Portfolio OK")
  return(list(RL_CVaR_Corr, RL_CVaR_Wt, RL_CVaR_Ret))
}
