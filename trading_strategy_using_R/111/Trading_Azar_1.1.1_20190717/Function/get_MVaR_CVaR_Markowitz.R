...
get_MVaR_CVaR_Markowitz <- function(Ret_Data){
  ...
  Wt_Markowitz_DT <- cbind(Wt_Markowitz, colnames(Ret_Data)[-1])
  ...
  RC_Markowitz_DT <- as.data.table(cbind(RC_Markowitz, colnames(Ret_Data)[-1]))
  ...
  MVaR_Markowitz_DT <- cbind(MVaR_Markowitz_DT, colnames(Ret_Data)[-1])
  colnames(MVaR_Markowitz_DT) <- c("MVaR", "Strategy")
  
  CVaR_Markowitz_DT <- as.data.table(CVaR_Markowitz)
  CVaR_Markowitz_DT <- cbind(CVaR_Markowitz_DT, colnames(Ret_Data)[-1])
