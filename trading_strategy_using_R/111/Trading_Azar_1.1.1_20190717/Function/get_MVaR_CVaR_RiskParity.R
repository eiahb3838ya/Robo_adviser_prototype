...
get_MVaR_CVaR_RiskParity <- function(Ret_Data){
  ...
  Wt_RiskParity_DT <- cbind(Wt_RiskParity_DT, colnames(Ret_Data)[-1])
  ...
  RC_RiskParity_DT <- as.data.table(cbind(RC_RiskParity, colnames(Ret_Data)[-1]))
  ...
  MVaR_Riskparity_DT <- cbind(MVaR_Riskparity_DT, colnames(Ret_Data)[-1])
  colnames(MVaR_Riskparity_DT) <- c("MVaR", "Strategy")
  
  CVaR_Riskparity_DT <- as.data.table(CVaR_Riskparity)
  CVaR_Riskparity_DT <- cbind(CVaR_Riskparity_DT, colnames(Ret_Data)[-1])
